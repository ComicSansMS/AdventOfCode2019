...
to the computer, this is always 4 (1 opcode + 3 parameters) for the add and multiply instructions. (The halt
instruction would increase the instruction pointer by 1, but it halts the program instead.)
...
void executeOpcode(IntegerProgram& p)
{
    ...
    switch (static_cast<Opcode>(p.memory[p.pc])) {
    case Opcode::Add:
        if (!checkParameters()) { p.pc = -3; break; }
