...
{
    return ranges::accumulate(masses | ranges::views::transform(rocket_equation), 0);
}

int rocket_equation_with_fuel(int mass)
{
    auto calculate_mass = [&mass]() { return mass = rocket_equation(mass); };
    return ranges::accumulate(ranges::views::generate(calculate_mass) |
...
{
    return ranges::accumulate(masses | ranges::views::transform(rocket_equation_with_fuel), 0);
}
